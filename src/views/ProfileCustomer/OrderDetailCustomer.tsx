...
<<<<<<< HEAD
    ...
    Clock, CheckCircle2, XCircle, Truck, Eye, ShieldAlert
} from "lucide-react";
import { api } from "@/lib/api";

// IMPORT COMPONENT MODAL BẢO HÀNH (Đổi đường dẫn nếu cần nhen)
...
const shippingConfig: Record<string, any> = {
...
export default function OrderDetailCustomer() {
    ...
    useEffect(() => {
        const fetchDetail = async () => {
            try {
                setLoading(true);
                // Sử dụng api.js thay vì fetch thuần
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-teal-50 to-white px-4 py-8">
            <div className="mx-auto w-full max-w-4xl space-y-6">
                ...
                {/* KHU VỰC THẺ BADGES + NÚT BẢO HÀNH */}
                <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm flex flex-wrap gap-3 items-center justify-between">
                    <div className="flex flex-wrap gap-3 items-center">
                        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl ${orderStatus.bg}`}>
                            <OrderStatusIcon className={`w-4 h-4 ${orderStatus.text}`} />
                                ...
                                return (
                                    <div key={step.key} className="flex flex-col items-center flex-1 min-w-[72px]">
                                        <div className="relative w-full flex items-center justify-center mb-3">
                                            ...
                                            <div className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center shrink-0 border-2 transition-all ${status === "done" ? "bg-teal-500 border-teal-500 text-white" : status === "active" ? "bg-white border-teal-500 shadow-lg shadow-teal-100" : "bg-white border-zinc-200"}`}>
                                        ...
                                        <p className={`text-xs font-semibold text-center leading-tight ${status === "done" ? "text-teal-600" : status === "active" ? "text-teal-700" : "text-zinc-400"}`}>
                                            {step.label}
                                        </p>
                                    </div>
                                );
                ...
                {/* Tổng tiền */}
                <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm space-y-2 mb-20">
            ...
            {/* NHÚNG COMPONENT MODAL BẢO HÀNH */}
            <WarrantyFormModal 
